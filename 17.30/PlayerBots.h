...
namespace PlayerBots {
    ...
    BehaviorTree* ConstructBehaviorTree() {
        ...
        {
            auto* Selector_Lobby = new BTComposite_Selector();
            Selector_Lobby->NodeName = "Lobby";

            {
                ...
                Decorator->Operator = EBlackboardCompareOp::NotEqual;
                Task->AddDecorator(Decorator);
                Selector_Lobby->AddChild(Task);
            }

            {
                auto* Task = new BTTask_SteerMovement(800.f, 2.f);
                auto* Decorator = new BTDecorator_CheckEnum();
                Decorator->SelectedKeyName = ConvFName(L"AIEvaluator_WarmupPlayEmote_ExecutionStatus");
                Decorator->IntValue = (int)EExecutionStatus::ExecutionAllowed;
                Task->AddDecorator(Decorator);

                auto* Service = new BTService_LobbyDance(12.f);
                Task->AddService(Service);
            ...
            {
                auto* Task = new BTTask_SteerMovement(600.f, 1.5f);
                auto* Decorator = new BTDecorator_CheckEnum();
                Decorator->SelectedKeyName = ConvFName(L"AIEvaluator_WarmupLootAndShoot_ExecutionStatus");
                Decorator->IntValue = (int)EExecutionStatus::ExecutionAllowed;
                Task->AddDecorator(Decorator);

                auto* Service_FindTarget = new BTService_LobbyFindTarget(5000.f, 1.5f);
                Task->AddService(Service_FindTarget);

                auto* Service_Shooting = new BTService_LobbyShooting(0.3f, 1.5f);
                Task->AddService(Service_Shooting);

                Selector_Lobby->AddChild(Task);
            }

            {
                auto* Task = new BTTask_SteerMovement(1000.f, 4.f);
                auto* Service = new BTService_HandleFocusing_ScanAroundOnly(0.5f, 200.f);
                Task->AddService(Service);
                Selector_Lobby->AddChild(Task);
            }

            Tree->AllNodes.push_back(Selector_Lobby);

            auto* Task_RunLobby = new BTTask_RunSelector();
            Task_RunLobby->SelectorToRun = Selector_Lobby;
            RootSelector->AddChild(Task_RunLobby);
        }
