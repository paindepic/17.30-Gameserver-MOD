...
{
	static auto fn = Utils::StaticLoadObject<UFunction>("/Script/Engine.DataTableFunctionLibrary.EvaluateCurveTableRow");
...
void SpawnLlamas()
{
	auto MapInfo = Utils::GetGameState()->MapInfo;
	...
	for (int i = 0; i < AmountOfLlamasToSpawn; i++)
	{
		...
		InitialSpawnTransform.Rotation = Utils::FRotToQuat(RandomYawRotator);
		InitialSpawnTransform.Scale3D = FVector(1, 1, 1);

		auto LlamaStart = Utils::SpawnActor22<AFortAthenaSupplyDrop>(MapInfo->LlamaClass.Get(), Location, RandomYawRotator);
		...
		auto Llama = Utils::SpawnActor22<AFortAthenaSupplyDrop>(MapInfo->LlamaClass.Get(), GroundLocation, RandomYawRotator);
